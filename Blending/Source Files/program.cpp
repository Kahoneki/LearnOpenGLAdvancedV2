...
unsigned int loadTexture(char const* path)
{
    ...
    if (data)
    {
        ...
        std::cerr << nrComponents << std::endl;
        glGenerateMipmap(GL_TEXTURE_2D);

        glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_WRAP_S, format == GL_RGBA ? GL_CLAMP_TO_EDGE : GL_REPEAT); //If transparent, don't repeat, since OpenGL will interpolate the top pixels with the bottom ones
        glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_WRAP_T, format == GL_RGBA ? GL_CLAMP_TO_EDGE : GL_REPEAT); //Same for T
